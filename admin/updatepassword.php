...
<section>
    ...
    <?php
        if(isset($_POST['s1']))
        {
            ...
            $query="select * from admin where id='$id' AND password='$cp'";
            ...
            if($run)
            {
                ...
                if($count==1)
                {
                    if($np==$cmp)
                    {
                            $query2="update admin set password='$np' where id='$id'";
                            $run2=mysqli_query($conn,$query2);
                            if($run2==TRUE)
                            {
                                echo "<script>window.location='manage-admin.php';</script>";
                                $_SESSION['add']="<p class='validation-green'>Wow !! Your password has been changed.</p>";
                            }
                    }
                    else
                    {
                        echo "<script>window.location='manage-admin.php';</script>";
                    $_SESSION['add']="<p class='validation-red'> Sorry !! Confirm Password Not Matched please Try again !</p>";
                    }
                }
                else
                {
                    echo "<script>window.location='manage-admin.php';</script>";
                    $_SESSION['add']="<p class='validation-red'> Sorry !! current password has been incorrected please Try again !</p>";
                }
            }   
        }
